      ...
      <header className="bg-white border-b border-surface-200 sticky top-0 z-40">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          <div className="flex items-center gap-4">
            <Link to="/user/dashboard" className="p-2 hover:bg-surface-100 rounded-xl transition-colors"><ArrowLeft className="w-5 h-5 text-text-secondary" /></Link>
          ...
          <div className="flex items-center gap-3">
            <Link to="/user/notifications" className="p-2 text-text-secondary hover:bg-surface-100 rounded-xl transition-colors"><Bell className="w-5 h-5" /></Link>
      ...
      <main className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {!loading && filtered.length === 0 && (
          <div className="bg-white rounded-2xl border border-surface-200 p-8 text-center">
            ...
            <p className="text-text-secondary mb-4">Accept offers on your scrap listings to schedule pickups.</p>
            <Link to="/user/listings/new" className="inline-flex items-center gap-2 px-6 py-3 gradient-brand text-white font-semibold rounded-xl hover:opacity-90 transition-all shadow-md shadow-brand-500/20">Create Listing</Link>
          </div>
        )}
        ...
        <div className="space-y-3">
          {filtered.map(pickup => {
            ...
            return (
              <Link key={pickup.id} to={`/user/listings/${pickup.listing_id}`}
